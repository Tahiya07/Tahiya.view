...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyTheme(t){
    if(t === 'dark'){
      ...
      if(dot){ dot.textContent = '🌙'; dot.style.transform = 'translateX(20px)'; }
    } else {
      root.removeAttribute('data-theme');
      if(toggle) toggle.setAttribute('aria-pressed','false');
      if(dot){ dot.textContent = '🌈'; dot.style.transform = 'translateX(0)'; }
    }
  }
  ...
  if(blobContainer){
    ...
    function animateBlobs(){
      ...
      blobs.forEach((b,i)=>{
        ...
        data.vx*=0.85; data.vy*=0.85; data.x+=data.vx; data.y+=data.vy;

        data.hue+=0.3; if(data.hue>360) data.hue-=360;
